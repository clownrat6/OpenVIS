...
_PREDEFINED_SPLITS_YTVIS_COCO = {
    "ytvis_2019_train2coco": ("ytvis_2019/train/JPEGImages", "ytvis_2019/ytvis_2019_train2coco.json"),
    "ytvis_2021_train2coco": ("ytvis_2021/train/JPEGImages", "ytvis_2021/ytvis_2021_train2coco.json"),
    # "ytvis2021_val2coco": ("coco/val2017", "coco/ytvis2021_valid2coco.json"),
    # "ovis_train2coco": ("coco/train2017", "coco/ovis_train2coco.json"),
    # "ovis_val2coco": ("coco/val2017", "coco/ovis_val2coco.json"),
}

YTVIS_COCO_CATEGORIES = [
...
def _get_ytvis_coco_instances_meta():
    thing_ids = [k["id"] for k in YTVIS_COCO_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in YTVIS_COCO_CATEGORIES if k["isthing"] == 1]
    assert len(thing_ids) == 101, len(thing_ids)
    # Mapping from the incontiguous YTVIS category id to an id in [0, 39]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in YTVIS_COCO_CATEGORIES if k["isthing"] == 1]
    ret = {
...
def register_all_ytvis_coco(root):
    for key, (image_root, json_file) in _PREDEFINED_SPLITS_YTVIS_COCO.items():
        # Assume pre-defined datasets live in `./datasets`.
        register_ytvis_instances(
            key,
            _get_ytvis_coco_instances_meta(),
