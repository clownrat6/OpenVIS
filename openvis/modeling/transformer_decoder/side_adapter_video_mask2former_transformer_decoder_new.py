...
class SideAdapterVideoMultiScaleMaskedTransformerDecoder(VideoMultiScaleMaskedTransformerDecoder):
    ...
    def forward(self, ms_feats, mask_feats, mask=None):

        bt, c_m, h_m, w_m = mask_feats.shape
        bs = bt // self.num_frames if self.training else 1
        t  = bt // bs
        mask_feats = mask_feats.view(bs, t, c_m, h_m, w_m)

        # h, w       = attn_feats.shape[-2:]
        attn_feats = self.attn_mlp(mask_feats)
        ...
        for i in range(self.num_feature_levels):
            size_list.append(ms_feats[i].shape[-2:])
            pos.append(self.pe_layer(ms_feats[i].view(bs, t, -1, size_list[-1][0], size_list[-1][1]), None).flatten(3))
            src.append(self.input_proj[i](ms_feats[i]).flatten(2) + self.level_embed.weight[i][None, :, None])
    ...
    def forward_prediction_heads(self, output, attn_feats, mask_feats, attn_mask_target_size):
        ...
        attn_biases   = torch.einsum("bqnc,btchw->btnqhw", attn_embed, attn_feats)

        outputs_mask  = torch.einsum("bqc,btchw->bqthw", mask_embed, mask_feats)
