...
class YTVOSeval:
    ...
    def _prepare(self):
        ...
        self._gts = defaultdict(list)       # gt for evaluation
        self._dts = defaultdict(list)       # dt for evaluation
        gt_v2c = defaultdict(list)
        for gt in gts:
            gt_v2c[gt['video_id']].append(gt['category_id'])
            self._gts[gt['video_id'], gt['category_id']].append(gt)
        for dt in dts:
            # for cat_id in gt_v2c[dt['video_id']]:
            #     self._dts[dt['video_id'], cat_id].append(dt)
    ...
    def evaluateVid(self, vidId, catId, aRng, maxDet):
        ...
        for g in gt:
            if (g['ignore'] == 1) or (g['avg_area'] < aRng[0] or g['avg_area'] > aRng[1]):
                g['_ignore'] = 1
            else:
                g['_ignore'] = 0
        ...
        for k, k0 in enumerate(k_list):
            ...
            for a, a0 in enumerate(a_list):
                ...
                for m, maxDet in enumerate(m_list):
                    # E (video results for a single class): [{}, ..., {}]
                    E = [self.evalImgs[Nk + Na + i] for i in i_list]
                    E = [e for e in E if not e is None]
                    if len(E) == 0:
                        continue
                    # E[0]['dtMatches'] (num of pr thresholds, num of dts with k-th class, 0-th video)
