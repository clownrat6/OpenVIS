...
from .augmentation_pseudo import build_pseudo_augmentation

from .coco_ytvis import COCO_TO_YTVIS_2019, COCO_TO_YTVIS_2021, COCO_TO_OVIS
...
class YTVISDatasetMapper:
    ...
    def __init__(
        ...
        *,
        ...
        sampling_frame_reverse: bool = False,
        num_classes: int = 40,
        src_dataset_name: str = "ytvis_2019_train",
        tgt_dataset_name: str = "ytvis_2019_train",
    ):
        """
    ...
    def from_config(cls, cfg, is_train: bool = True):
        ...
        sampling_frame_reverse = cfg.INPUT.SAMPLING_FRAME_REVERSE

        if is_train:
            tgt_dataset_name = cfg.DATASETS.TRAIN[-1]
        else:
            tgt_dataset_name = cfg.DATASETS.TEST[-1]
        ...
        ret = {
            ...
            "sampling_frame_reverse": sampling_frame_reverse,
    ...
    def select_frames(self, video_length):
        ...
        if self.sampling_frame_ratio < 1.0:
            ...
            if self.sampling_frame_range * 2 + 1 == self.sampling_frame_num:
                ...
                if self.sampling_frame_reverse and random.random() < 0.5:
    ...
    def __call__(self, dataset_dict):
        ...
        dataset_dict["dataset_name"] = self.tgt_dataset_name
        dataset_dict["frame_ids"] = selected_idx
        dataset_dict["image"] = []
        ...
        for frame_idx in selected_idx:
            ...
            annos = [
                utils.transform_instance_annotations(obj, transforms, image_shape) for obj in _frame_annos
            ...
            # accu_mask = np.zeros(image_shape)
            # import matplotlib.pyplot as plt
            # plt.subplot(121)
            # plt.imshow(image)
            # plt.subplot(122)
            # for m in instances.gt_masks.tensor:
            #     accu_mask[m > 0] = 1
            # plt.imshow(accu_mask)
            # plt.savefig('2.png')
            # print(instances)
...
class CocoClipDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        for _ in range(self.sampling_frame_num):
            ...
            # USER: Implement additional transformations if you have other types of data
            annos = [
                utils.transform_instance_annotations(obj, transforms, image_shape) for obj in _img_annos
