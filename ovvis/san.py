...
class SAN(VideoMaskFormer):
    ...
    def forward(self, batched_inputs):
        ...
        outputs['pred_logits'] = self.adapter.get_sim_logits(clip_bk_feats, outputs['class_attn_biases'], text_feats).mean(dim=1)

        if self.training:
            if 'aux_outputs' in outputs:
                for idx, pred in enumerate(outputs["aux_outputs"]):
                    outputs['aux_outputs'][idx]['pred_logits'] = self.adapter.get_sim_logits(clip_bk_feats, pred['class_attn_biases'], text_feats).mean(dim=1)
...
class SANOnline(MinVIS):
    ...
    def forward(self, batched_inputs):
        ...
        outputs['pred_logits'] = self.adapter.get_sim_logits(clip_bk_feats, outputs['class_attn_biases'], text_feats)

        if self.training:
            if 'aux_outputs' in outputs:
                for idx, pred in enumerate(outputs["aux_outputs"]):
                    outputs['aux_outputs'][idx]['pred_logits'] = self.adapter.get_sim_logits(clip_bk_feats, pred['class_attn_biases'], text_feats)
...
@META_ARCH_REGISTRY.register()
class TemporalSANOnline(SANOnline):
    """
    Main class for mask classification semantic segmentation architectures.
    """
    ...
    def __init__(self, max_iter_num, **kwargs):
        ...
        self.resampler = TemporalInstanceResampler(window_size=self.window_size)
        self.iter = 0
    ...
    def forward(self, batched_inputs):
        ...
        if not self.training and self.window_inference:
            ...
            # mask_feats = einops.rearrange(mask_feats, '(b t) c h w -> b t c h w', b=len(batched_inputs))
            # attn_feats = einops.rearrange(attn_feats, '(b t) n c h w -> b t n c h w', b=len(batched_inputs))
    ...
    def run_window_inference(self, images_tensor, clip_bk_feats, clip_mg_feats, text_feats):
        ...
        for i in range(iters):
            ...
            # sementer inference
            features = self.backbone(images_tensor[start_idx:end_idx])
            image_outputs = self.sem_seg_head(features, extra_feats=[x[start_idx:end_idx] for x in clip_mg_feats])
            ...
            overall_clip_bk_feats.append({k: v[:, start_idx:end_idx] if 'cls_token' in str(k) else v[start_idx:end_idx] for k, v in clip_bk_feats.items()})
