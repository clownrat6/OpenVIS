...
from .modeling.minvis import MinVIS, batch_video_match_via_embeds
from .modeling.clip_adapter import build_clip_adapter
from .modeling.resampler import BasicTemporalInstanceResampler
...
class SimpleBaseline(VideoMaskFormer):
    ...
    @configurable
    def __init__(self, *, video_clip_adapter: nn.Module, frame_clip_adapter: nn.Module, clip_ensemble: bool, clip_ensemble_weight: float, **kwargs):
        super().__init__(**kwargs)
        self.video_clip_adapter          = video_clip_adapter
    ...
    def from_config(self, cfg):
        ...
        video_clip_adapter = build_clip_adapter(cfg.MODEL.VIDEO_CLIP_ADAPTER)
        frame_clip_adapter = build_clip_adapter(cfg.MODEL.FRAME_CLIP_ADAPTER)
        # open-vocabulary
        args_dict["video_clip_adapter"]   = video_clip_adapter
        args_dict["frame_clip_adapter"]   = frame_clip_adapter
        args_dict["clip_ensemble"]        = cfg.MODEL.VIDEO_CLIP_ADAPTER.CLIP_ENSEMBLE
        args_dict["clip_ensemble_weight"] = cfg.MODEL.VIDEO_CLIP_ADAPTER.CLIP_ENSEMBLE_WEIGHT
    ...
    def forward(self, batched_inputs):
        ...
        text_features = self.video_clip_adapter.get_text_features_w_noobj(class_names)
        outputs["pred_logits"] = self.video_clip_adapter.get_sim_logits(
            text_features, self.video_clip_adapter.normalize_feature(outputs["pred_logits"]))
        if self.training:
            if "aux_outputs" in outputs.keys():
                for i in range(len(outputs["aux_outputs"])):
                    outputs["aux_outputs"][i]["pred_logits"] = self.video_clip_adapter.get_sim_logits(
                        text_features,
                        self.video_clip_adapter.normalize_feature(outputs["aux_outputs"][i]["pred_logits"]))
    ...
    def open_vocabulary_inference(self, scores: torch.Tensor, masks: torch.Tensor, frames: torch.Tensor, class_names):
        if len(scores) > 0:
            if self.clip_ensemble:
                ...
                clip_cls = torch.cat(clip_cls).softmax(dim=-1)
...
@META_ARCH_REGISTRY.register()
class TemporalSimpleBaselineOnline(MinVIS):
    """
    Main class for mask classification semantic segmentation architectures.
    """

    @configurable
    def __init__(self, max_iter_num, video_clip_adapter: nn.Module, frame_clip_adapter: nn.Module, clip_ensemble: bool, clip_ensemble_weight: float, **kwargs):
        super().__init__(**kwargs)
        ...
        self.resampler = BasicTemporalInstanceResampler(window_size=self.window_size)
        self.iter = 0
    ...
    def forward(self, batched_inputs):
        ...
        text_feats = self.video_clip_adapter.get_text_features_w_noobj(class_names)

        if not self.training and self.window_inference:
            outputs = self.run_window_inference(images.tensor, text_feats)
        else:
            self.backbone.eval()
            ...
            # pred_logits: (1, bt, q, c) -> (b, t, q, c) -> (b, t, q, c)
            logits = einops.rearrange(image_outputs['pred_logits'][0], '(b t) q c -> b t q c', b=len(batched_inputs))
            image_outputs['pred_logits'] = self.video_clip_adapter.get_sim_logits(text_feats, self.video_clip_adapter.normalize_feature(logits))
            ...
            outputs = self.resampler(image_outputs['ms_feats'], image_outputs['ms_pos'], image_outputs['size_list'], frame_embeds, image_outputs['mask_feats'], self.video_clip_adapter, text_feats)
        ...
        if self.training:
            ...
            mask_cls_result, mask_pred_result = self.post_processing(outputs, images.tensor.shape[-2:])
    ...
    def run_window_inference(self, images_tensor, text_feats):
        ...
        for i in range(iters):
            ...
            del image_outputs['pred_masks'], image_outputs['pred_logits']
            del image_outputs['aux_outputs']
        ...
        outputs = self.resampler(overall_ms_src, overall_ms_pos, size_list, overall_frame_embeds, overall_mask_feats, self.video_clip_adapter, text_feats)

        del overall_ms_src, overall_ms_pos, overall_frame_embeds, overall_mask_feats, text_feats
        torch.cuda.empty_cache()
