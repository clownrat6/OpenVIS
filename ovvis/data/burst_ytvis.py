...
__all__ = ["register_burst_ytvis_instances"]
...
def _get_burst_ytvis_instances_meta():
    thing_ids = [k["lvis_id"] for k in BURST_CATEGORIES if k["isthing"] == 1]
    thing_colors = [k["color"] for k in BURST_CATEGORIES if k["isthing"] == 1]
    assert len(thing_ids) == 482, len(thing_ids)
    # Mapping from the incontiguous YTVIS category id to an id in [0, 39]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in BURST_CATEGORIES if k["isthing"] == 1]
    ret = {
...
def register_burst_ytvis_instances(name, metadata, json_file, image_root):
    """
...
_PREDEFINED_SPLITS_BURST = {
    "burst_ytvis_train": ("burst/frames/train",
                    "burst/train_ytvis.json"),
    "burst_ytvis_val":   ("burst/frames/val",
                    "burst/val_ytvis.json"),
    "burst_ytvis_test":  ("burst/frames/test",
                    "burst/test_ytvis.json"),
}


def register_all_burst_ytvis(root):
    for key, (image_root, json_file) in _PREDEFINED_SPLITS_BURST.items():
        # Assume pre-defined datasets live in `./datasets`.
        register_burst_ytvis_instances(
            key,
            _get_burst_ytvis_instances_meta(),
...
register_all_burst_ytvis(_root)
