# Modified by Zesen Cheng from: https://github.com/facebookresearch/detr/blob/master/models/position_encoding.py
"""
...
class PositionEmbeddingSine1D(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        not_mask = ~mask
        embed = not_mask.cumsum(0, dtype=torch.float32)
        if self.normalize:
            eps = 1e-6
            embed = embed / (embed[-1:, :] + eps) * self.scale

        dim_t = torch.arange(self.num_pos_feats, dtype=torch.float32, device=x.device)
        dim_t = dim_t.div(2, rounding_mode='floor')        
...
class PositionEmbeddingSine3D(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        dim_t_z = dim_t_z.div(2, rounding_mode='floor')
        dim_t_z = self.temperature ** (2 * dim_t_z / (self.num_pos_feats * 2))
