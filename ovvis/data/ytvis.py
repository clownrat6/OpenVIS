...
def load_ytvis_json(json_file, image_root, dataset_name=None, extra_annotation_keys=None):
    ...
    if dataset_name is not None:
        ...
        cat_ids = cats.keys()
        thing_classes = [cats[i] for i in sorted(cats.keys())]
    ...
    for (vid_dict, anno_dict_list) in vids_anns:
        ...
        for frame_idx in range(record["length"]):
            ...
            for anno in anno_dict_list:
                ...
                if ("train" in dataset_name) and not (_bboxes and _segm and _bboxes[frame_idx] and _segm[frame_idx]):
                    continue
