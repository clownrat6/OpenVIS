...
class BasicTemporalInstanceResampler(nn.Module):

    def __init__(self, clip_dim=512, hidden_dim=256, feed_dim=2048, nheads=8, nlayers=6, window_size=10):
        super().__init__()
        # define Transformer decoder here
        self.num_heads = nheads
        self.num_layers = nlayers
        ...
        # output FFNs
        self.class_embed = MLP(hidden_dim, clip_dim * 2, clip_dim, 2)
        self.mask_embed = MLP(hidden_dim, hidden_dim, hidden_dim, 3)

        self.adapter = None
        self.text_feats = None

    def forward(self, src, pos, size_list, frame_embeds, mask_feats, adapter=None, text_feats=None):
        self.adapter = adapter
        self.text_feats = text_feats
        ...
        for i in range(self.num_layers):
            ...
            if not self.training and window_flag:
                ...
                outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads_list(tgt_list, mask_feats, attn_mask_target_size=size_list[next_level_index])
                temporal_tgt = torch.cat(tgt_list, dim=1)
            else:
                temporal_tgt = self.resample_infer(temporal_tgt, i, attn_mask, src[level_index], pos[level_index])
                outputs_class, outputs_mask, attn_mask = self.forward_prediction_heads(temporal_tgt, mask_feats, attn_mask_target_size=size_list[next_level_index])
    ...
    def forward_prediction_heads(self, output, mask_feats, attn_mask_target_size):
        output = self.decode_norm(output).transpose(1, 0)

        mask_embed = self.mask_embed(output)
        masks = torch.einsum("bqc,bchw->bqhw", mask_embed, mask_feats)

        embeds = self.class_embed(output)    
        logits = self.adapter.get_sim_logits(self.text_feats, self.adapter.normalize_feature(embeds))
    ...
    def forward_prediction_heads_list(self, output, mask_feats, attn_mask_target_size):
        logits_list = []
        masks_list = []
        attn_masks_list = []
        for o, m in zip(output, mask_feats):
            o = self.decode_norm(o).transpose(1, 0)

            mask_embed = self.mask_embed(o)
            masks = torch.einsum("bqc,bchw->bqhw", mask_embed, m)

            embeds = self.class_embed(o)
            logits = self.adapter.get_sim_logits(self.text_feats, self.adapter.normalize_feature(embeds))
